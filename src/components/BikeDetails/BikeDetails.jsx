  ...
  useEffect(() => {
    const fetchBike = async () => {
      ...
      try {
        ...
        const images =
          found.images?.map(
            (img) => `${API_BASE_URL}/uploads/${img.s3Key}`
          ) || [];

        setBike({
          id: found._id,
          name: found.model_name || "hiten",
          model: found.segment || "hiten",
          price: variant.ex_showroom_price
            ? `₹ ${variant.ex_showroom_price.toLocaleString("en-IN")}`
            : "hiten",
          image: images[0] || "hiten",
        });
        ...
        setBikeDetails({
          ...
          tyre: variant.tubeless ? "Tubeless" : "hiten",
          maxPower: variant.max_power || "hiten",
          gearBox: variant.gearbox || "hiten",
          clutch: variant.clutch || "hiten",
          kerbWeight: variant.kerb_weight_kg || "hiten",
          steering: "Handle Bar",

          engineSpecs: {
            "Engine CC": variant.engine_cc || "hiten",
            "Engine Type": variant.engine_type || "hiten",
            "Max Power": variant.max_power || "hiten",
            "Max Torque": variant.max_torque || "hiten",
          },

          transmission: {
            Transmission: variant.transmission || "hiten",
            Gearbox: variant.gearbox || "hiten",
            Clutch: variant.clutch || "hiten",
          },

          dimensions: {
            Length: variant.length_mm || "hiten",
            Width: variant.width_mm || "hiten",
            Height: variant.height_mm || "hiten",
            Wheelbase: variant.wheelbase_mm || "hiten",
            "Ground Clearance": variant.ground_clearance_mm || "hiten",
            "Seat Height": variant.seat_height_mm || "hiten",
          },

          chassis: {
            "Front Suspension": variant.suspension_front || "hiten",
            "Rear Suspension": variant.suspension_rear || "hiten",
          },

          brakesSpecs: {
            "Front Brake": variant.brakes_front || "hiten",
            "Rear Brake": variant.brakes_rear || "hiten",
          },

          tyres: {
            "Front Tyre": variant.tyres_front || "hiten",
            "Rear Tyre": variant.tyres_rear || "hiten",
          },
